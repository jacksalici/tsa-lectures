"""
Custom styling for matplotlib plots.

To use, create an instance of MyStyle with the desired theme ('slate' or 'zinc'),
then call the `apply()` method to style your plots.    

Works with single or multiple Axes objects.

Avaiable color palettes for plot elements are defined in the Palette enum.
...
class MyStyle:
    ...
    def __init__(self, theme: str = "zinc", zoom: float = 1.0):
    ...
    def _apply_style_to_ax(self, ax: Axes, legend_frame_on: bool, colored_labels: bool, colored_bg: bool) -> None:
        ...
        for spine in ax.spines.values():
            ...
            spine.set_linewidth(1.2 * self.zoom)

        if colored_labels:
            ax.tick_params(axis="both", colors=self.dark_1, width=1.2 * self.zoom)

        for label in ax.get_xticklabels() + ax.get_yticklabels():
            label.set_fontfamily(self._get_font_family())
            label.set_fontsize(12 * self.zoom)

        if ax.get_title():
            ax.title.set_fontfamily(self._get_font_family())
            ax.title.set_fontsize(16 * self.zoom)
        ...
        for axis_label in [ax.xaxis.label, ax.yaxis.label]:
            if axis_label.get_text():
                ...
                axis_label.set_fontsize(14 * self.zoom)
                if colored_labels:
        ...
        if legend:
            ...
            legend.get_frame().set_linewidth(1.2 * self.zoom)
            for label in legend.get_texts():
                label.set_fontfamily(self._get_font_family())
                label.set_fontsize(12 * self.zoom)
            legend.get_title().set_fontfamily(self._get_font_family())
            legend.get_title().set_fontsize(14 * self.zoom)
